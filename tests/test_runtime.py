...
async def test_openai_agents_bridge_fallback():
    ...
    msg = Message(from_agent="Planner", to_agent="Summarizer", content="summarize this task")
...
async def test_langgraph_bridge_fallback():
    ...
    msg = Message(from_agent="Planner", to_agent="Router", content="build an OCR image service")
    result = await agent.think(msg)
    assert result is not None
